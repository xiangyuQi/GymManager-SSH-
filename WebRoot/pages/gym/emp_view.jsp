...
<section id="widget-grid" class="">
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
		  	...
		  	<div class="form-group">
		    	部门<input type="text" style="width:120px;" class="form-control" name="Q_LKS_dept.dname">
		  	</div>
		  	<div class="form-group">
		    	职位 <k:dictSelect name="Q_EQI_job.id" className="form-control" itemName="职位" headerKey=""/>
		  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			url: 'emp/list',
			caption: '<i class="glyphicon glyphicon-list"></i>员工信息列表',
			colModel: [
				...
				{label: '姓名', name: 'name', width: 30, editable: true},
				{label: '所属部门', name: 'dept.dname', width: 50},
				{label: '职位', name: 'job.itemValue', width: 50},
		...
		top.layer.params = {};
			layer.open({
				...
				area: ['500px', '350px'],
				maxmin: true,
			    content: ['pages/gym/emp_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.post('./emp/save', form.serialize(), function(json){
			    		//刷新表格
			    		if(json.success) {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./emp/delete', {ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
