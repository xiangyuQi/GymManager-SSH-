...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setColWidth();
		//todo：+2
		$(eg).css("width",(grid.width+2)+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+ts.p.id+"'>&#160;</div>");
		if(ts.p.scrollPopUp) {
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			//todo: +10, +2, +2, +2
			nwidth = parseInt(nwidth,10) + 10; 
			$t.grid.width = $t.p.width = nwidth;
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("width",(nwidth+2)+"px");
			$("#gview_"+$.jgrid.jqID($t.p.id)).css("width",(nwidth+2)+"px");
			$($t.grid.bDiv).css("width",(nwidth+2) +"px");
